...
class AxiSymmetricFiniteStrain( Element ):
...
#------------------------------------------------------------------------------
  ...
  def getTangentStiffness ( self, elemdat ):
    ...
    for iData in sData:
      ...
      geomstiff = zeros(shape=(n,n))
      
      for i,dpi in enumerate(iData.dhdx):
        for j,dpj in enumerate(iData.dhdx):
          geomstiff[2*i,2*j] += s4[0]*dpi[0]*dpj[0] + s4[1]*dpi[1]*dpj[1]
          geomstiff[2*i,2*j] += s4[2]*iData.h[i]*iData.h[j]/(r*r)                 
          geomstiff[2*i,2*j] += s4[3]*(dpi[0]*dpj[1] + dpi[1]*dpj[0])
          
          geomstiff[2*i+1,2*j+1] += s4[0]*dpi[0]*dpj[0] + s4[1]*dpi[1]*dpj[1]      
          geomstiff[2*i+1,2*j+1] += s4[3]*(dpi[0]*dpj[1] + dpi[1]*dpj[0])
          
      elemdat.stiff += geomstiff * weight
